...
router.post('/restore/:id', authenticateToken, checkRole(['manager']), usersController.restoreUser); // Route mới để khôi phục người dùng
router.put('/change-password', authenticateToken, usersController.changePassword);
router.put('/approve/:id',authenticateToken, checkRole(['manager']),  usersController.approveUser);
